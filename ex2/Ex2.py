...
class MagicSquareProblem(GeneticAlgorithmProblem):
    ...
    def mutation(self, mutation_rate=0.05):
        ...
        # randomize number 0-1
        rand_n = float(np.random.random(1)[0])
        # generate an even number from random number
        # will change
        pick_n = int(min(2 * (mutation_rate // rand_n), self.square.size))
        pick_index = np.random.choice(self.square.size, size=pick_n, replace=False)
        flatten_square = self.square.flatten()
        for i in range(0, len(pick_index), 2):
            tmp_value = flatten_square[pick_index[i]]
            flatten_square[pick_index[i]] = flatten_square[pick_index[i+1]]
            flatten_square[pick_index[i+1]] = tmp_value
